# DELETION OF TAIL OF SINGLY LINKED LIST

'''
Given the head of a singly linked list, delete the last node (tail) of the linked list and return the head.
If the linked list is empty or has only one node, return null.

Note: The tail is the last node of the linked list.
'''
...
result8 = deleteTail(head8)
print("Output: head -> 999 -> 888") # Result: head -> 999 -> 888

# Test Case 9
print("Test Case 9:") # Input: head -> 0 -> 1 -> 2
head9 = ListNode(0, ListNode(1, ListNode(2)))
result9 = deleteTail(head9)
print("Output: head -> 0 -> 1") # Result: head -> 0 -> 1
...
result10 = deleteTail(head10)
print("Output: head -> 1 -> 2 -> 3 -> 4 -> 5 -> 6 -> 7 -> 8 -> 9") # Result: head -> 1 -> 2 -> 3 -> 4 -> 5 -> 6 -> 7 -> 8 -> 9